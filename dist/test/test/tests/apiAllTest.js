...
import { Album, AlbumExtended, Artist, ArtistExtended, Playlist, Song, Thumbnails, Video, } from '../../resources/generalTypes/index.js';
...
test('searchUnsorted', async (t) => {
    ...
    t.true(WalkUtility
        .walkAndAssertShape(resultButLink, expected), 'resultButLink has expected shape');
    t.end();
});
...
test('api_getPlaylistParser', async (t) => {
    ...
    t.true(WalkUtility
        .walkAndAssertShape(resultSmall, expected[0]), 'api_getPlaylistURLParser (small playlist) result has expected shape');
    t.true(WalkUtility
        .walkAndAssertShape(resultLarge, expected[0]), 'api_getPlaylistURLParser (large playlist) result has expected shape');
    t.true(WalkUtility
        .walkAndAssertShape(quad_flexColumn, expected[0]), 'api_getPlaylistURLParser (quad_flexColumn playlist) result has expected shape');
    t.end();
});
